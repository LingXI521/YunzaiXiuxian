...
import { existplayer, exist_najie_thing, ForwardMsg, isNotNull, Getmsg_battle,Gaodenyuansulun } from '../Xiuxian/xiuxian.js'
...
export class WDT extends plugin {
    ...
    async biwu(e) {
        ...
        let last_biwu_time = await redis.get("xiuxian:player:" + A + ":last_biwu_time");//获得上次打劫的时间戳,
        ...
        if (B_player.当前血量 <= (B_player.血量上限/1.2)) { e.reply(`${B_player.名号} 血量未满，不能趁人之危哦`); return; }
        if (A_player.当前血量 <= (A_player.血量上限/1.2)) { e.reply(`你血量未满，对方不想趁人之危`); return; }
