...
import { existplayer, exist_najie_thing, ForwardMsg, isNotNull,Write_player,Gaodenyuansulun } from '../Xiuxian/xiuxian.js'
import { Read_player } from '../Xiuxian/xiuxian.js'
import { Add_najie_thing, Add_灵石, Add_HP,Add_血气 } from '../Xiuxian/xiuxian.js'
...
export class Battle extends plugin {
    ...
    async Dajie(e) {
      ...
      return;
        ...
        if (A_player.修为 < 0) { e.reply(`还是闭会关再打劫吧`); return; }
        if (B_player.当前血量 < 20000) { e.reply(`${B_player.名号} 重伤未愈,就不要再打他了`); return; }
        if (B_player.灵石 < 30002) { e.reply(`${B_player.名号} 穷得快赶上水脚脚了,就不要再打他了`); return; }
        ...
        if (msg.find(item => item == A_win)) {
            ...
            if (lingshi>=B_player.灵石){lingshi = B_player.灵石/2}
            A_player.灵石+=lingshi
            B_player.灵石-=lingshi
            A_player.血气+=qixue
            A_player.魔道值+=mdz;
    ...
    let msg = [];
        ...
        if (cnt % 2 == 0) {
            ...
            if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·鬼帝功")>-1 && Random > 0.20 && cnt==0) {msg.push(`${A_player.名号} 使用【鬼剑】然暴起冲向 ${B_player.名号}，速度之快，对方根本反应不过来。`);伤害 = Math.trunc(伤害 * 1.1 + 100000);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("伪八品·影杀")>-1 && Random > 0.20 && cnt==0) {msg.push(`${A_player.名号} 使用影杀！突然暴起冲向 ${B_player.名号}，速度之快，对方根本反应不过来。`);伤害 = Math.trunc(伤害 * 1 + 100000);}
            else if (Random > 0.50 && cnt==0) {msg.push(`你找准时机！突然暴起冲向 ${B_player.名号}，但是被对方反应过来了`);伤害 = 0;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·八荒剑法")>-1 && cnt==2) {msg.push(`${A_player.名号} 使用八荒剑法【斩八荒！】`);伤害 = Math.trunc(伤害 * 1.2);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·天星")>-1 && cnt==4) {msg.push(`${A_player.名号} 使用天星【天动万象！】`);伤害 = Math.trunc(伤害 * 1.2 + 200000);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·太素")>-1 && cnt==4) {msg.push(`${A_player.名号} 使用太素【太素】`);伤害 = Math.trunc(伤害 * 1.2 + 200000);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·心禅不灭诀")>-1 && cnt==4) {msg.push(`${A_player.名号} 使用八品·心禅不灭诀【万剑归宗】`);伤害 *= 1.25;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·太皇经")>-1 && cnt==2) {msg.push(`${A_player.名号} 使用八品·太皇经【无量仙功】开始聚集仙气`);伤害 *= 0.9;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·太皇经")>-1 && cnt==2) {msg.push(`${A_player.名号} 使用八品·太皇经 聚集完成！【皇极斩！】`);伤害 = Math.trunc(伤害 * 1.25 + 500000);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("伪八品·二重梦之㱬")>-1 && cnt==6) {msg.push(`${A_player.名号} 使用二重梦之㱬【梦轮】`);伤害 *= 1.25;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("九品·第一魔功")>-1 && cnt==2) {msg.push(`${A_player.名号} 使用第一魔功【噬天！】`);伤害 = Math.trunc(伤害 * 1.15 + 300000);}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("九品·第一魔功")>-1 && Random < 0.03) {msg.push(`${B_player.名号} 使用了第一魔功【魔转！】你的伤害被转走了大部分`);伤害 *= 0.5;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("伪九品·魔帝功")>-1 && Random < 0.3 && cnt==2) {msg.push(`${B_player.名号} 使用了魔帝功【吞噬】你的伤害被吸收了`);伤害 *= -0.1;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·避空")>-1 && cnt==4) {msg.push(`${B_player.名号} 使用了避空【遁空！】`);伤害 *= 0.5;}
            else if (Random < 0.06) {msg.push(`你找到了 ${B_player.名号} 的破绽！这一下无处可逃！`);伤害 *=1.3 ;}
            else if (A_player.灵根.name==="轮回道体"&& cnt==0) {msg.push(`${A_player.名号} 使用了先天神通，轮回之力需要时间准备`);伤害 *=0.8 ;}
            else if (A_player.灵根.name==="轮回道体"&& cnt==4) {msg.push(`${A_player.名号} 使用了先天神通，轮回之力崩泄而出！`);伤害 *=1.5 ;}
            else if (B_player.灵根.name==="破虚踏星体"&& Random > 0.98) {msg.push(`${B_player.名号}使用了先天神通 【破虚】！你的伤害无法命中！`);伤害 *=0 ;}
            else if (B_player.灵根.type==="转生"&& Random > 0.98) {msg.push(`${B_player.名号}使用了转生神通 【轮墓】！你的伤害无法生效！`);伤害 *=0 ;}
            else if (A_player.灵根.name==="灭道杀神体"&& cnt==12) {msg.push(`${A_player.名号}使用了先天神通 【杀破神】！`);伤害 *=3 ;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·桃花神功")>-1 && cnt==4 && Random > 0.66) {msg.push(`${A_player.名号} 使用了【三生桃花！】你的攻击慢慢变成了漫天桃花飞舞。`);伤害 *= -0.2;}
            else if (Random > 0.94) {msg.push(`你的攻击被 ${B_player.名号} 破解了`);伤害 *=0.6 ;}
            else if (Random > 0.9) {msg.push(`你的攻击被 ${B_player.名号} 接下来了`);伤害 *=0.8 ;}
...
${B_player.名号}冻结中`);
        ...
        if (cnt % 2 == 1) {
            ...
            let Random = Math.random();        
            if (Random < 0.06) {msg.push(`你找到了 ${A_player.名号} 的破绽！这一下无处可逃！`);伤害 *=1.3 ;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·八荒剑法")>-1 && cnt==3) {msg.push(`${B_player.名号} 使用八荒剑法【斩八荒！】`);伤害 *= 1.2;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·太皇经")>-1 && cnt==3) {msg.push(`${B_player.名号} 使用八品·太皇经【无量仙功】开始聚集仙气`);伤害 *= 0.9;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·太皇经")>-1 && cnt==9) {msg.push(`${A_player.名号} 使用八品·太皇经 聚集完成！【皇极斩！】`);伤害 = Math.trunc(伤害 * 1.25 + 500000);}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·天星")>-1 && cnt==5) {msg.push(`${B_player.名号} 使用天星【天动万象！】`);伤害 = Math.trunc(伤害 * 1.2 + 200000);}
            else if (A_player.学习的功法&&B_player.学习的功法.indexOf("八品·太素")>-1 && cnt==4) {msg.push(`${B_player.名号} 使用太素【太素】`);伤害 = Math.trunc(伤害 * 1.2 + 200000);}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·心禅不灭诀")>-1 && cnt==5) {msg.push(`${B_player.名号} 使用八品·心禅不灭诀【万剑归宗】`);伤害 = Math.trunc(伤害 * 1.25);}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("伪八品·二重梦之㱬")>-1 && cnt==7) {msg.push(`${B_player.名号} 使用二重梦之㱬【梦轮】`);伤害 = Math.trunc(伤害 * 1.25);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·避空")>-1 && cnt==5) {msg.push(`${A_player.名号} 使用了避空【遁空！】`);伤害 = Math.trunc(伤害 * 0.5);}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("九品·第一魔功")>-1 && cnt==3) {msg.push(`${B_player.名号} 使用第一魔功【噬天！】`);伤害 = Math.trunc(伤害 * 1.15 + 300000);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("九品·第一魔功")>-1 && Random < 0.03) {msg.push(`${A_player.名号} 使用了第一魔功【魔转！】你的伤害被转走了大部分`);伤害 = Math.trunc(伤害 * 0.5);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("伪九品·魔帝功")>-1 && Random < 0.3 && cnt==3) {msg.push(`${A_player.名号} 使用了魔帝功【吞噬】你的伤害被吸收了`);伤害 *= -0.1;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·桃花神功")>-1 && cnt==5 && Random > 0.66) {msg.push(`${B_player.名号} 使用了【三生桃花！】你的攻击慢慢变成了漫天桃花飞舞。`);伤害 *= -0.2;}
            else if (B_player.灵根.name==="轮回道体"&& cnt==0) {msg.push(`${B_player.名号} 使用了先天神通，轮回之力需要时间准备`);伤害 *=0.8 ;}
            else if (B_player.灵根.name==="轮回道体"&& cnt==4) {msg.push(`${B_player.名号} 使用了先天神通，轮回之力崩泄而出！`);伤害 *=1.5 ;}
            else if (A_player.灵根.name==="破虚踏星体"&& Random > 0.98) {msg.push(`${A_player.名号}使用了先天神通 【破虚】！你的伤害无法命中！`);伤害 *=0 ;}
            else if (A_player.灵根.type==="转生"&& Random > 0.98) {msg.push(`${A_player.名号}使用了转生神通 【轮墓】！你的伤害无法生效！`);伤害 *=0 ;}
            else if (B_player.灵根.name==="灭道杀神体"&& cnt==13) {msg.push(`${B_player.名号}使用了先天神通 【杀破神】！`);伤害 *=3 ;}
            else if (Random > 0.94) {msg.push(`你的攻击被 ${A_player.名号} 破解了`);伤害 = Math.trunc(伤害 * 0.6) ;}
            else if (Random > 0.9) {msg.push(`你的攻击被 ${A_player.名号} 接下来了`);伤害 = Math.trunc(伤害 * 0.8) ;}
...
export async function mjzd_battle(A_player, B_player) {
    ...
    while (A_player.当前血量 > 0 && B_player.当前血量 > 0) {
        if (cnt % 2 == 0) {
            ...
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·八荒剑法")>-1 && cnt==2) {msg.push(`${A_player.名号} 使用八荒剑法【斩八荒！】`);伤害 *= 1.2;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·天星")>-1 && cnt==4) {msg.push(`${A_player.名号} 使用天星【天动万象！】`);伤害 = Math.trunc(伤害 * 1.2 + 200000);}
            else if (A_player.学习的功法&&B_player.学习的功法.indexOf("八品·太素")>-1 && cnt==4) {msg.push(`${B_player.名号} 使用太素【太素】`);伤害 = Math.trunc(伤害 * 1.2 + 200000);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·心禅不灭诀")>-1 && cnt==4) {msg.push(`${A_player.名号} 使用八品·心禅不灭诀【万剑归宗】`);伤害 *= 1.25;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·太皇经")>-1 && cnt==2) {msg.push(`${A_player.名号} 使用八品·太皇经【无量仙功】开始聚集仙气`);伤害 *= 0.9;}
            ...
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("伪九品·第一魔功")>-1 && cnt==2) {msg.push(`${A_player.名号} 使用第一魔功【噬天！】`);伤害 = Math.trunc(伤害 * 1.1 + 300000);}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("伪九品·第一魔功")>-1 && Random < 0.02) {msg.push(`${B_player.名号} 使用了第一魔功【魔转！】你的伤害被转走了大部分`);伤害 *= 0.5;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("伪九品·魔帝功")>-1 && Random < 0.3 && cnt==2) {msg.push(`${B_player.名号} 使用了魔帝功【吞噬】你的伤害被吸收了`);伤害 *= -0.1;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·避空")>-1 && cnt==4) {msg.push(`${B_player.名号} 使用了避空【遁空！】`);伤害 *= 0.5;}
            else if (Random < 0.06) {msg.push(`你找到了 ${B_player.名号} 的破绽！这一下无处可逃！`);伤害 *=1.3 ;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·桃花神功")>-1 && cnt==4 && Random > 0.66) {msg.push(`${A_player.名号} 使用了【三生桃花！】你的攻击慢慢变成了漫天桃花飞舞。`);伤害 *= -0.2;}
...
${A_player.名号}攻击了${B_player.名号}，${ifbaoji(baoji)}造成伤害${伤害}，${B_player.名号}剩余血量${B_player.当前血量}`);
        ...
        if (cnt % 2 == 1) {
            ...
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("八品·心禅不灭诀")>-1 && cnt==5) {msg.push(`${B_player.名号} 使用八品·心禅不灭诀【万剑归宗】`);伤害 *= 1.25;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("伪八品·二重梦之㱬")>-1 && cnt==7) {msg.push(`${B_player.名号} 使用二重梦之㱬【梦轮】`);伤害 *= 1.25;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·避空")>-1 && cnt==5) {msg.push(`${A_player.名号} 使用了避空【遁空！】`);伤害 *= 0.5;}
            else if (B_player.学习的功法&&B_player.学习的功法.indexOf("伪九品·第一魔功")>-1 && cnt==3) {msg.push(`${B_player.名号} 使用第一魔功【噬天！】`);伤害 = Math.trunc(伤害 * 1.1 + 300000);}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("伪九品·第一魔功")>-1 && Random < 0.02) {msg.push(`${A_player.名号} 使用了第一魔功【魔转！】你的伤害被转走了大部分`);伤害 *= 0.5;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("伪九品·魔帝功")>-1 && Random < 0.3 && cnt==3) {msg.push(`${A_player.名号} 使用了魔帝功【吞噬】你的伤害被吸收了`);伤害 *= -0.1;}
            else if (A_player.学习的功法&&A_player.学习的功法.indexOf("八品·桃花神功")>-1 && cnt==5 && Random > 0.66) {msg.push(`${A_player.名号} 使用了【三生桃花！】你的攻击慢慢变成了漫天桃花飞舞。`);伤害 *= -0.2;}
            else if (Random > 0.94) {msg.push(`你的攻击被 ${A_player.名号} 破解了`);伤害 *=0.6 ;}
            else if (Random > 0.9) {msg.push(`你的攻击被 ${A_player.名号} 接下来了`);伤害 *=0.8 ;}
            伤害 = Math.trunc(伤害);
            A_player.当前血量 -= 伤害;
            if (A_player.当前血量 < 0) { A_player.当前血量 = 0 }
            msg.push(`第${Math.trunc(cnt / 2) + 1}回合：
